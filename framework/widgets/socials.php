<?php
class funvita_Social_Widget extends WP_Widget {
    function __construct() {
        parent::__construct(
                'social_widget', // Base ID
                esc_html__('Socials', 'funvita'), // Name
                array('description' => esc_html__('Social Widget', 'funvita'),) // Args
        );
    }
        ...
        <div class="bt-social-wrap">
            <?php
				for ($i = 1; $i <= 12; $i++) {
					if($icon_social[$i]){
					?>
					<a data-icon="<?php echo esc_attr($icon_social[$i]); ?>" href="<?php echo esc_url($link_social[$i]); ?>">
    ...
    function form($instance) {
		...
		?>
		<p>
            <label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e('Title', 'funvita'); ?></label>
        ...
        for ($i = 1; $i <= 12; $i++) {
            ?>
            <p>
                <label for="<?php echo esc_url($this->get_field_id('icon_social_' . $i)); ?>"><?php echo esc_html__('Social Icon', 'funvita').$i; ?></label>
            ...
            <p>
                <label for="<?php echo esc_url($this->get_field_id('link_social_' . $i)); ?>"><?php echo esc_html__('Social Link', 'funvita').$i; ?></label>
        ...
        <?php } ?>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('el_class')); ?>"><?php _e('Extra Class', 'funvita'); ?></label>
...
/**
 * Class funvita_Social_Widget
 */
function funvita_register_social_widget() {
    register_widget('funvita_Social_Widget');
}
add_action('widgets_init', 'funvita_register_social_widget');
?>
