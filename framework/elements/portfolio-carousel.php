...
class WPBakeryShortCode_bt_portfolio_carousel extends WPBakeryShortCode {
	...
	protected function content( $atts, $content = null ) {
		...
		$data_script = "jQuery(document).ready(function($) {
							if ($('.bt-portfolio-carousel-element .owl-carousel').length) {
					...
					<?php
						if($rows == 1){
							while ( $wp_query->have_posts() ) { $wp_query->the_post();
								require get_template_directory().'/framework/elements/portfolio_layouts/'.$layout.'.php';
							}
						}else{
...
vc_map(array(
	'name' => esc_html__('Portfolio Carousel', 'funvita'),
    ...
    'params' => array(
		...
		array(
			'type' => 'dropdown',
			'heading' => esc_html__('Nav & Dots Style', 'funvita'),
			'param_name' => 'nav_dots',
			'value' => array(
				esc_html__('Default', 'funvita') => 'nav-dots-default',
				esc_html__('Style 1', 'funvita') => 'nav-dots-style1'
			),
			'group' => esc_html__('Owl Setting', 'funvita'),
			'description' => esc_html__('Select nav and dots style display in this element.', 'funvita')
		),
		array(
			'type' => 'dropdown',
