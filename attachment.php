...
$blog_titlebar = isset($funvita_options['blog_titlebar']) ? $funvita_options['blog_titlebar']: true;
if($blog_titlebar) funvita_titlebar();
?>
				...
				<div class="bt-content <?php echo esc_attr($content_class); ?>">
					...
					while ( have_posts() ) : the_post();
						if ( wp_attachment_is_image( get_the_ID() ) ) {
							...
							echo '<img src="'.esc_attr($att_image[0]).'" width="'.esc_attr($att_image[1]).'" height="'.esc_attr($att_image[2]).'"  class="attachment-medium" alt="'.esc_attr__('Thumbnail', 'funvita').'" />';
						}
					endwhile;
					?>
				</div>
